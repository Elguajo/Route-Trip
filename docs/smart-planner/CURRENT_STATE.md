...
- Every `TripDay` persists a validated local `start_time`/`end_time` window, defaulted for old and new days to `09:00`–`18:00`. `DayTimeBudget` derives its maximum usable minutes without storing a duplicated budget. Existing category, item, and day API contracts serialize and persist the new additive fields; Angular types mirror them, but no time-settings UI exists yet.
...
- Day budget-aware allocation/order, timeline/summary, overflow reporting, route segments, or persisted optimisation summaries.
